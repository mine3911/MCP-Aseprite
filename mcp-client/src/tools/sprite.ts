...
export const createSpriteTools = (
  server: McpServer,
  asepriteClient: AsepriteClient
) => {
  server.tool(
    "create-sprite",
    "在Aseprite中创建sprite",
  ...
  server.tool(
    "draw-pixel",
    "在Aseprite中的当前Sprite中绘画像素点。有三种模式可用：1、单点像素绘制（传入一个点坐标）；2、连线绘制（传入两个点坐标，绘制这两个点形成的线）；3、填充模式（传入一个点的坐标，填充绘制这个点所在的区域）",
    {
      jsonStr: z.string().describe(
        '数组类型的json串，格式为: [{"drawType":"1","targetLayer":"1","targetFrame":"1","createFrameMode":"1","posStr":"","color_r":"0","color_g":"0","color_b":"0","color_a":"0"},{...}]' + 
